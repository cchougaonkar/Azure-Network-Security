...
Function Get-AZResourcesHash {
    ...
    Write-Host "Total Resources matching interestedResourcesQuery: $($AzResourcesList.TotalResources)"
    $maxRecords = 1000
    $skipRecords = 0
    Do {
...
# For running the workflows in parallel by subscription
#
...
#>    
    ...
    [string]$outputAzResourcesHashFile = "$outputFileLocation\AzureResourcesHash.json"
    ...
    $AzResourcesHash = Get-AzResourcesHash -query $interestedResourcesQuery -keyName 'dnsEndPoint' -outputFile $outputAzResourcesHashFile
...
<#
Process-CNameList
    Iterate over each item in the CNAME list -
        if item has an FQDN, check if the precalculated hash map contains the fqdn as a key. If found, add to matching resources, otherwise add to missing resources

 This means two things have to be correct: the CNAMEs have to fetched correctly and the Hash map must contain all resources owned by the user
 
 CNAME list generation -
    Depending upon input parameters fetch from either the input csv file or from subscription regex in parameter
    Mode 1 - Input csv -
        Extract object from the input file -> add a zoneName, fqdn and resource provider field for each record
        Call: Process-CNameList $inputCNameList
    
    Mode 2 - Subscription regex
        Comes into play if the -FetchDnsRecordsFromAzureSubscription parameter is set
        Fetch all subscriptions that match the input subscription regex
        Get-AzResourcesLst -query $dnszoneQuery -> returns list of all dnsZones in the subscriptions matching regex
        filter out zones that matter to the user (zones matching the zone regex) -> $interestedZones
        Iterate over all subscriptions -
            Select-AzSubscription $subscription
            filter zones from $interestedZones that match the current subscription -> $interestedZones1
            Get list of CNAME records from $interestedZones1 (Get-AzCNameToDnsMap) -> $cNameRecords: To fetch iterate over each zone in zone list and get dns cname records from that zone
            iterate over each CNAME record in $cNameRecords -
                add subscriptionName and subscriptionId to the record
                add the record to $AzDnsCNameRecordSets
        Call: Process-CNameList $AzDnsCNameRecordSets

 Hashmap creation -
    Get-AzResourcesHash -> takes in a query and a keyName as parameters
            query: azure graph query to fetch all resources whose subscriptionId matches the regex in the parameter, and resource type is one that has a DNS endpoint 
                    -> extent this result to have a dnsEndpoint value that indicates the fdqn for that resource 
                    -> extend this result to contain a resourceProvider column
            keyName: 'dnsEndpoint'

            Get the result of the AzGraph query with pagination
            For each resource in the result -> if the hashmap already contains the Key add the resource to the list against that key, otherwise add the (key, list) pair to the hashmap
#>
